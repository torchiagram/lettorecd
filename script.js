...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function forcePlay() {
        ...
        widget.play();
        // Fallback per iOS: riprova dopo un breve delay
    ...
    function loadTrack(plIdx, sIdx) {
        ...
        // Appena finisce il caricamento (o quasi), forziamo il play
        widget.bind(SC.Widget.Events.READY, forcePlay);
    }

    // GESTIONE POPUP (Sblocco Audio)
    function unlockAndStart() {
        audioUnlocked = true;
        permPopup.style.display = 'none';
        // Se c'è un CD inserito, avvialo, altrimenti sblocca solo il widget
        if (currentPlaylistIdx !== -1) {
            forcePlay();
        } else {
            widget.play(); // Sblocca il contesto audio
            setTimeout(() => widget.pause(), 100);
        }
    }
    ...
    // CONTROLLI (Tutti usano forcePlay)
